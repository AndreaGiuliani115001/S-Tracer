...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Recupera la foto associata al componente
        $foto = Componenti::getFoto($conn, $id);
        if ($foto) {
        ...
        // Elimina il componente (le relazioni sono gestite da ON DELETE CASCADE)
        if (Componenti::delete($conn, $id)) {
            $conn->commit();
            header('Location: ../../dashboard_componenti.php?progetto_id=' . $progetto_id . '&attivita_id=' . $attivita_id . '&stato=' . $stato . '&linea_produzione_id=' . $linea_produzione_id . '&success=Componente eliminato');
        } else {
            $conn->rollBack();
